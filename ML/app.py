...
def process_canvas(canvas_result):
    """Processa os dados do canvas convertendo RGBA diretamente para o formato EMNIST."""
    ...
    # Se a imagem tiver 4 canais (RGBA)
    if len(img_array.shape) == 3 and img_array.shape[2] == 4:
        # Pega a escala de cinza baseada no canal RGB
        img_gray = cv2.cvtColor(img_array[:, :, :3], cv2.COLOR_RGB2GRAY)
        # Se os canais RGB forem zero mas houver traço no Alpha, usa o Alpha
        if np.max(img_gray) == 0 and np.max(img_array[:, :, 3]) > 0:
    ...
    # Se a tela estiver totalmente vazia (sem traço)
    ...
    # Normaliza entre 0 e 1 e formata para matriz (1, 784)
...
st.write("Desenhe o caractere no quadro preto. Clique em **Classificar** caso o resultado não apareça instantaneamente.")
...
with tab1:
    ...
    with col_canvas:
        ...
        btn_vf = st.button("Classificar V/F", key="btn_vf_action")
    ...
    with col_pred:
        ...
        if img_processed is not None:
            ...
            if model:
                ...
                # Mapeia 1 -> V, 0 -> F (ou saídas equivalentes do modelo)
...
with tab2:
    ...
    with col_canvas:
        ...
        btn_num = st.button("Classificar Dígito", key="btn_num_action")
...
with tab3:
    ...
    with col_canvas:
        ...
        btn_let = st.button("Classificar Letra", key="btn_let_action")

...
def process_canvas(canvas_result):
    """Extrai e processa a imagem do canvas tratando a exceção do streamlit_drawable_canvas."""
    if canvas_result is None:
        return None

    # Trata a exceção lançada internamente ao acessar .image_data antes do tempo
    try:
    ...
    # Extrai intensidade do traço cobrindo RGBA ou RGB
    ...
    # Se nada foi desenhado na tela (tudo zero)
    if np.max(img_gray) == 0:
        return None

    # Redimensiona e alinha para o padrão EMNIST (28x28)

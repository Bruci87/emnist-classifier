...
def process_canvas(canvas_result):
    """Extrai e processa a imagem do canvas sem falhar na detecção do traço."""
    ...
    # Extrai o valor máximo entre todos os canais (R, G, B, Alpha)
    if len(img_array.shape) == 3:
        img_gray = np.max(img_array, axis=2)
    else:
        img_gray = img_array

    # Se a soma dos pixels for 0, nada foi desenhado
    ...
    # Transposição para alinhar a rotação das colunas/linhas com a base EMNIST
    img_transposed = cv2.transpose(img_resized)

    # Normalização entre 0 e 1 e flattening para (1, 784)
...
def process_canvas(canvas_result):
    ...
    # Extrai o valor máximo entre todos os canais (R, G, B, Alpha) para garantir
    # que qualquer traço feito na tela seja capturado, independente do modo de cor
    if len(img_array.shape) == 3:
        img_gray = np.max(img_array, axis=2)
    else:
        img_gray = img_array
